...
public class UserService {
    ...
    @Transactional
    public UserResponse signUpCourier(CourierSignUpRequest request) {
        // 1. 비밀번호 일치 확인
        if (!request.getPassword().equals(request.getConfirmPassword())) {
            throw new IllegalArgumentException("비밀번호와 비밀번호 확인이 일치하지 않습니다.");
        }
        ...
        // 3. 비밀번호 암호화 및 배송원 엔티티 생성
        String encodedPassword = passwordEncoder.encode(request.getPassword());
        User courier = User.createCourier(
                request.getName(),
                request.getEmail(),
                encodedPassword,
                request.getPhoneNumber(),
                ...
                request.getPreferredWeight()
        );

        User savedCourier = userRepository.save(courier);
        return UserResponse.from(savedCourier);
    }
}
