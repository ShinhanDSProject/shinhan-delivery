...
이 문서는 `shinhan-delivery` 프로젝트에서 **무결점 결함 0개(Zero-Defect Quality Gate)를 사수하고 개발자와 AI의 코드 품질을 자동 검증하기 위해 구축하는 테스트 하네스(`./scripts/verify.sh`)의 의사결정 판단 정책 및 초엄격 하네스 수칙**을 정리한 팀 가이드북입니다.
