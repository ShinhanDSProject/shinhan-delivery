...
이 문서는 `shinhan-gaecheokja` 프로젝트에서 **서비스 간 강한 결합을 해소하고 비동기 이벤트 발행/구독 모델을 통해 시스템 확장성을 확보하는 이벤트 주도 아키텍처(EDA)**의 가이드북입니다.
...
```bash
./gradlew test --tests "com.example.shinhangaecheokja.test.service.DeliveryServiceTest"
```
