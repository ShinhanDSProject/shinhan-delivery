...
metadata:
  version: "1.0.0"
  ssot_owner: "docs/methodologies/clean-architecture.md"
...
이 문서는 `shinhan-gaecheokja` 프로젝트의 **핵심 비즈니스 로직이 외부 기술(DB, 프레임워크, Web UI)에 오염되지 않고 순수함을 유지하도록 돕는 클린/헥사고날 아키텍처** 방법론의 가이드북입니다.
...
```bash
./gradlew test --tests "com.example.shinhangaecheokja.architecture.ArchUnitTest"
```
