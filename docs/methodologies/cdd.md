...
이 문서는 `shinhan-gaecheokja` 프로젝트에서 **프론트엔드와 백엔드 간, 혹은 마이크로서비스 간 REST API 명세 계약(Contract)을 미리 체결하고 독립적으로 병렬 개발하는 계약 주도 개발** 방법론의 가이드북입니다.
...
```bash
./gradlew test --tests "com.example.shinhangaecheokja.architecture.RestApiConventionTest"
```
