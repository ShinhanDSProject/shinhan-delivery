...
* **1단계: 설계 PR (Design-First PR)**
  * 코드를 한 줄도 작성하지 않은 상태에서, 아래의 설계 산출물만 마크다운 파일로 작성하여 PR을 생성합니다.
  * 리뷰어(동료, AI, 멘토)로부터 데이터베이스 관계 설정이 올바른지, API 설계에 빈틈이 없는지 먼저 확인받습니다.
* **2단계: 구현 PR (Implementation PR)**
  * 설계 리뷰가 통과되면, 해당 설계를 바탕으로 실제 코드 구현 및 테스트 코드를 작성하여 최종 PR을 올리고 병합합니다.
