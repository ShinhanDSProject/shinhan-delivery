...
metadata:
  version: "1.1.0"
  ssot_owner: "docs/beginners/초급-개발자-태스크-분할-가이드.md"
...
> [!NOTE]
> 본 가이드북은 [docs/architecture/SSOT-문서화-정책-가이드.md](./SSOT-문서화-정책-가이드.md) 전문 기술 문서화 표준 규격과 [code-convention.md](../code-convention.md) 코딩 수칙을 준수합니다.
...
* 참조 가이드: [docs/architecture/REST-API-설계-규격-가이드.md](./REST-API-설계-규격-가이드.md)
...
* 참조 가이드: [docs/architecture/전역-예외-처리-규격-가이드.md](./전역-예외-처리-규격-가이드.md)
...
#### Thymeleaf SSR 화면을 구현하는 경우

* 초기 화면에 필요한 데이터는 Web Controller가 Service를 호출해 DTO로 변환하고 `Model`에 담습니다.
* 템플릿은 `th:each`, `th:text`, `th:if`로 Model 값을 렌더링합니다. 페이지가 열린 뒤 동일 데이터를 다시 `fetch()`하여 화면을 조립하지 않습니다.
* 로그인 회원 데이터는 `@AuthenticationPrincipal CustomUserDetails`로 회원을 식별하고, principal이 없으면 `/login`으로 리다이렉트합니다.
* 저장·수정·삭제와 실시간 갱신은 SSR 초기 조회와 별개입니다. 프로젝트의 REST API 및 JavaScript 유지 범위는 공식 컨벤션을 따릅니다.
* 상세 규칙은 [code-convention.md §18](../../code-convention.md)와 [SSR 쿠키 폴백 인증 ADR](../adr/0002-SSR-쿠키-폴백-인증-체계.md)을 확인하고, 화면별 흐름은 [전체 유저 플로우 가이드](../architecture/전체-유저-플로우-가이드.md)를 확인합니다.
...
* 참조 가이드: [docs/harness/PR-리뷰어-3분-족보-가이드.md](./PR-리뷰어-3분-족보-가이드.md)
...
* [테스트 하네스 가이드 (docs/harness/테스트-하네스-및-LLM-가이드.md)](./테스트-하네스-및-LLM-가이드.md)
```
