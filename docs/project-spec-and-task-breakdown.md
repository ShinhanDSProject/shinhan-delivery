...
metadata:
  version: "1.0.0"
  ssot_owner: "docs/project-spec-and-task-breakdown.md"
...
* **FE-018 매칭 대기 (`matching-wait`):** WebSocket/Polling 매칭 상태 감시, 매칭 취소 (`DELETE /api/deliveries/{id}/matching`)
...
## 👥 5. 5인 팀원별 역할 정의 및 태스크 매핑 매트릭스 (5-Member Team Allocation)

팀원 5명의 역량과 도메인 특성을 살려 **역할을 명확히 정의하고 스프린트 태스크를 상호 시너지 구조로 분배**합니다:

```mermaid
graph TD
    Leader["👑 팀장 (Member A)<br/>아키텍처/보안/통제탑"] --- BE1["💻 백엔드 A (Member B)<br/>배송/매칭 핵심 로직"]
    Leader --- BE2["⚡ 백엔드 B (Member C)<br/>실시간/포인트/주소 API"]
    Leader --- FE1["📱 프론트엔드 A (Member D)<br/>온보딩/홈/지도 SDK"]
    Leader --- FE2["🎨 프론트엔드 B (Member E)<br/>인증/결제/실시간 추적 UI"]
```

### 👤 Member A (팀장 & 백엔드 코어 / 아키텍처)
* **주요 역할:** 전체 아키텍처 수호, Spring Security + JWT 보안, 공통 예외/하네스 관리, PR 리뷰어 통제탑
* **담당 모듈:** 모듈 2 (인증/회원가입 백엔드), 모듈 5 (결제 PIN 보안 검증), ADR 수록 및 CI/CD 파이프라인 관리

### 👤 Member B (백엔드 도메인 A - 배송 & 매칭)
* **주요 역할:** 핵심 배송 서비스 비즈니스 로직, 요금 산정 엔진, 매칭 대기/완료 이벤트 로직
* **담당 모듈:** 모듈 4 (배송 신청/요금 산정 API), 모듈 6 (배송원 매칭 API), 모듈 8 (배송 내역/상세/취소 API)

### 👤 Member C (백엔드 도메인 B - 실시간/포인트/주소)
* **주요 역할:** WebSocket 실시간 위치 추적 백엔드, 포인트 지갑 & PG 연동, 알림/마이페이지/주소 CRUD API
* **담당 모듈:** 모듈 3 (알림센터 API), 모듈 7 (실시간 추적 WebSocket), 모듈 9 (포인트 충전/내역 API), 모듈 10 (주소/프로필 API)

### 👤 Member D (프론트엔드 A - 홈/지도/배송신청 UI)
* **주요 역할:** 모바일/웹 메인 UX, 온보딩 워크스루, 홈 대시보드, 지도 SDK (카카오맵/TMap) 연동
* **담당 모듈:** FE-001~002 (스플래시/워크스루), FE-007 (홈 대시보드), FE-009~014 (주소/지도/카테고리/픽업가이드 UI)

### 👤 Member E (프론트엔드 B - 인증/결제/추적 UI)
* **주요 역할:** 소셜/이메일 로그인 UI, 6자리 결제 PIN 커스텀 키패드, 지도 위 실시간 위치 마커 애니메이션 UI
* **담당 모듈:** FE-003~006 (로그인/회원가입 UI), FE-015~017 (결제 PIN 키패드 UI), FE-018~021 (매칭/실시간 위치 마커 UI), FE-025~031 (포인트/마이페이지 UI)

---

## 🧪 6. 실증 검증 명령어 (Verification Commands)
...
## 🛠️ 7. 다음 추천 액션: GitHub Issues 자동 생성
