...
* PR을 올리면 1분 이내에 **Gemini AI 코드 리뷰어**가 한국어 튜터링 톤으로 변경 코드를 정밀 검사해 개선 피드백 코멘트를 줍니다.
* AI와 팀원 피드백을 수용하여 필요한 코드를 수정한 뒤 푸시합니다.
* 최종 승인(Approve)을 득한 후 GitHub 상에서 **`Squash and Merge`** 버튼을 눌러 작업을 완료합니다!
