...
### 7단계: AI 피드백 검토, 컨벤션 자산화 및 최종 병합 (Continuous Feedback Loop)

코드 리뷰와 빌드 검증은 단순한 지적질이 아니라 **팀 전체의 기술 자산과 규격을 승화시키는 지속적 피드백 루프(Continuous Feedback Loop)**입니다.

```mermaid
graph LR
    A["1. 기능 구현 & 패턴 설계"] --> B["2. CI 빌드 & AI 리뷰 피드백"]
    B --> C["3. 리뷰 반영 & /review 재검증"]
    C --> D["4. 컨벤션 문서(code-convention.md) 동기화 자산화"]
    D --> E["5. Squash and Merge 최종 병합"]
    E -->|"다음 개발 피드백 루프로 선순환"| A
```

* **피드백 수집 및 반영:** PR을 올리면 1분 이내에 **Gemini AI 코드 리뷰어**가 한국어 튜터링 톤으로 변경 코드를 정밀 검사해 개선 피드백 코멘트를 자동으로 작성합니다.
