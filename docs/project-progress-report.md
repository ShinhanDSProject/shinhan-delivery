...
graph TD
    subgraph "신한 개척자 개발팀 (Dev Team)"
        ...
        MS["kms7522 / 강민성<br/>(배송/매칭 Core & 실시간 UI)"]
...
### 👨‍💻 kms7522 / 강민성 (배송 & 매칭 코어 API / 실시간 UI / 알림)
- **배송 내역 목록/상세 조회 API (`#205`)**, **실시간 배송 브로드캐스트 & WebSocket 권한 검증 (`#189`, `#207`)**, **배송 완료 사진 증거 API (`#184`)**.
- **Figma 연동 웹 UI 구현 (`#103`, `#104`, `#105`, `#165`, `#183`, `#187`)**: Yeeun 님의 Figma 디자인을 반영한 지도 SDK 주소 입력, 실시간 추적 & 문앞 사진 확인 UI 구축.
...
- **Sprint 2 (홈 & 배송 신청):** 요금 산정, 지도 SDK 연동 UI, 카테고리, 이미지 업로드 완료 (`#98` ~ `#105`)
- **Sprint 4 (실시간 추적 & 내역 API):** 배송내역 API (`#114`), WebSocket 위치 추적 (`#113`), 사진 증거 (`#182`, `#184`), 추적 UI (`#116`) 완료
...
- **동시성 락(Lock) 검증 테스트:** 다수 기사가 동시 매칭 수락 시 중복 매칭이 발생하지 않음을 Multi-thread 테스트로 검증.
