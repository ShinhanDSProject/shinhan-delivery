...
2. [**로컬 개발 환경 및 자동화 도구 사용 가이드 (docs/harness/개발자-환경-설정-가이드.md)**](./개발자-환경-설정-가이드.md)의 **3번(로컬 테스트용 더미 데이터 자동 적재)**을 참고하여, 프로젝트 루트에 `.env` 파일을 만들고 아래 코드를 입력해 저장합니다:
   ```env
...
3. 터미널에서 아래 명령을 실행해 프로젝트를 띄웁니다.
   ...
   ./gradlew bootRun
   ```
4. 실행 중 문제가 생긴다면 주저하지 말고 [**로컬 개발 트러블슈팅 가이드 (docs/harness/트러블슈팅-가이드.md)**](./트러블슈팅-가이드.md)를 열어 해결책을 따라 해 보세요.
...
2. 로컬 코드 스타일의 자동 검증을 강제하기 위해 pre-commit hook인 **Lefthook**을 설치 및 등록합니다:
   ...
   * 설치 후 프로젝트 루트 경로 터미널에서 아래 명령을 1회 실행하여 활성화합니다:
     ...
     lefthook install
     ```
3. [**Git Flow 및 커밋 컨벤션 가이드 (docs/harness/Git-Flow-및-커밋-컨벤션.md)**](./Git-Flow-및-커밋-컨벤션.md)를 정독하여, 브랜치를 왜 나누어 쓰는지와 왜 **`Squash and Merge`** 방식으로만 PR을 병합하는지 그 흐름을 이해합니다.
4. [**Flyway 데이터베이스 마이그레이션 가이드 (docs/architecture/Flyway-마이그레이션-가이드.md)**](./Flyway-마이그레이션-가이드.md)를 읽고, 데이터베이스 테이블 변경을 자바 코드가 아닌 SQL 스크립트 기반으로 추적하는 규칙을 배웁니다.

### 3일차: 기능 설계 과정 이해하기
1. [**기능 개발 전 설계 단계 프로세스 가이드 (docs/harness/기능-설계-2단계-PR-절차-가이드.md)**](./기능-설계-2단계-PR-절차-가이드.md)를 정독하여, 코딩을 바로 시작하지 않고 '설계 PR'을 먼저 올리는 프로세스를 익힙니다.
...
### 2단계: 설계 PR 제출 (1차 PR)
* `docs/harness/기능-설계-2단계-PR-절차-가이드.md` 양식에 맞추어 `docs/design/기능이름-design.md` 파일을 작성합니다. (사용자 스토리, ERD, API 명세, 작업 분할 체크리스트 기재)
...
> 💡 **명심해 주세요:**  
> 처음이라 빌드가 실패하거나 깃 명령어가 꼬이는 것은 지극히 정상적인 성장 과정입니다. 막히는 부분이 생기면 언제든 `docs/harness/트러블슈팅-가이드.md`를 찾아보거나 주저하지 말고 동료 교육생 및 멘토에게 질문하세요! 여러분의 도전을 응원합니다!
