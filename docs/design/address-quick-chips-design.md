...
sequenceDiagram
    ...
    Repo-->>Service: 본인 소유 주소 목록
    Service-->>API: 주소 목록
    API-->>Page: AddressResponse DTO 목록
    Customer->>Page: 입력 대상과 별칭 칩 선택
    alt 유효한 저장 좌표가 있음
        Page->>Page: 저장 좌표 우선 사용
    else 저장 좌표가 없거나 비정상
...
3. 칩을 누르면 유효한 저장 좌표를 우선 사용하고, 없거나 비정상인 경우에만 주소를 좌표로 변환한 뒤 선택한 입력 대상만 갱신한다. 좌표 변환까지 실패하면 기존 입력값을 유지하며 배송 생성이나 결제는 수행하지 않는다.
...
- **Trade-off:** 현재 주소 테이블과 응답 DTO에는 좌표가 없어 선택 시 지도 API 호출이 한 번 필요하다. 향후 응답에 좌표가 추가되면 유효한 저장 좌표를 우선 재사용하며, 현재는 좌표를 영구 저장해 생길 수 있는 데이터 불일치와 마이그레이션 비용을 피한다.
