# 설계서: 쿠폰 2단계: 고객용 쿠폰 다운로드/발급 및 내 쿠폰함 관리

이 문서는 고객이 발급 가능한 쿠폰 목록을 조회하고, 직접 쿠폰을 다운로드/발급받아 자신의 쿠폰함에서 조회 및 관리 기능의 요구사항, API, ERD 및 비즈니스 로직을 정의합니다.

## 1. 요구사항 정의서 (User Story)

### 1.1 고객의 쿠폰 다운로드/발급
* **User Story:**
  우리는 **고객(user)**으로서, 혜택을 받기 위해 이벤트/상품 페이지에서 발급 가능한 쿠폰을 조회하고 직접 다운로드할 수 있기를 원한다.
* **성공 기준 (Acceptance Criteria):**
    1. 발급 기간 내에 유효한 쿠폰에 대해 발급 요청 시, 1인당 제한 수량 및 전체 선착순 수량을 초과하지 않았다면 정상적으로 UserCoupon 데이터가 생성되고 발급 수량이 증가한다.
    2. 이미 본인의 최대 발급 허용 수량만큼 발급받은 상태에서 재요청할 경우, `400 Bad Request`와 함께 "이미 발급받은 쿠폰입니다" 예외 메시지를 반환한다.
    3. 쿠폰의 전체 발급 수량이 모두 차감/소진된 상태에서 발급 요청 시, `400 Bad Request`와 함께 "쿠폰이 모두 소진되었습니다" 예외 메시지를 반환한다.
    4. 발급 시작 전이거나 발급 종료일이 지난 쿠폰에 대해 발급 요청 시, `400 Bad Request`와 함께 "쿠폰 발급 기간이 아닙니다" 예외 메시지를 반환한다.
    5. 동일한 선착순 쿠폰에 대해 순간적으로 수백~수천 건의 동시 발급 요청이 들어와도 정해진 totalQuantity를 초과하여 초과 발급(Over-issuance)되지 않아야 한다. 
---
