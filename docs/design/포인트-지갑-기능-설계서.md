# 설계서: 포인트 지갑 및 충전 (Point Wallet & Payments)

이 문서는 회원 포인트 지갑 생성, 충전, 사용, 충전 이력, 멱등성 처리를 다룹니다.

---

## 1. 요구사항 정의 (User Story)

* **User Story:**
  우리는 **회원(Member)** 으로서 배송 결제와 정산을 위해 개인 포인트 지갑을 보유하고, 충전 및 차감을 수행하기를 원한다.
* **수용 기준 (Acceptance Criteria):**
  1. 회원당 포인트 지갑은 1개만 가진다.
  2. 충전/차감 금액은 0보다 커야 한다.
  3. 잔액 부족 차감은 `400 Bad Request` 로 거절한다.
  4. 인증 회원 기준 충전 API는 `Idempotency-Key` 로 중복 충전을 막아야 한다.
  5. 충전 성공 시 충전 이력이 저장되어야 한다.
  6. 배송 자동 취소 환불은 원 배송 요청 ID와 사유를 기록하고 동일 멱등 키로 한 번만 반영한다.

---

## 2. ERD 설계

```mermaid
erDiagram
    MEMBER ||--o| POINT_WALLET : owns
    POINT_WALLET ||--o{ POINT_HISTORY : records

    POINT_WALLET {
        Long id PK
        Long member_id FK
        Long balance
        Long version
    }

    POINT_HISTORY {
        Long id PK
        Long member_id FK
        Long wallet_id FK
        Long amount
        Long balance_after
        String type
        String payment_method
        String idempotency_key
        LocalDateTime created_at
        Long reference_id "환불 원 배송 ID"
        String description "환불 사유"
    }
```

### 2.1 PointWallet DB 설계

| 컬럼명 | 타입 | 제약조건 | 설명 |
| --- | --- | --- | --- |
| `id` | `BIGINT` | PK | 포인트 지갑 식별자 |
| `member_id` | `BIGINT` | FK, UNIQUE, NOT NULL | 지갑 소유 회원 ID |
| `balance` | `BIGINT` | NOT NULL | 현재 포인트 잔액 |
| `version` | `BIGINT` | NOT NULL | 동시성 제어용 버전 값 |

### 2.2 PointHistory DB 설계

| 컬럼명 | 타입 | 제약조건 | 설명 |
| --- | --- | --- | --- |
| `id` | `BIGINT` | PK | 포인트 이력 식별자 |
| `member_id` | `BIGINT` | FK, NOT NULL | 이력 소유 회원 ID |
| `wallet_id` | `BIGINT` | FK, NOT NULL | 대상 포인트 지갑 ID |
| `amount` | `BIGINT` | NOT NULL | 충전 또는 차감 금액 |
| `balance_after` | `BIGINT` | NOT NULL | 처리 후 잔액 |
| `type` | `VARCHAR` | NOT NULL | `CHARGE`, `USE`, `REFUND` 또는 `COURIER_COMPENSATION` |
| `payment_method` | `VARCHAR` | NULL 허용 | 충전 시 결제 수단 |
| `idempotency_key` | `VARCHAR` | NULL 허용 | 멱등 처리 키 |
| `created_at` | `DATETIME` | NOT NULL | 이력 생성 시각 |
| `reference_id` | `BIGINT` | NULL 허용 | 환불의 원 배송 요청 ID |
| `description` | `VARCHAR` | NULL 허용 | 환불 처리 사유 |

---

## 3. API 명세

### 3.1 포인트 지갑 생성
* **엔드포인트:** `POST /api/v1/point-wallets`

### 3.2 지갑 기준 충전
* **엔드포인트:** `POST /api/v1/point-wallets/{walletId}/charge`
* **비고:** 기존 관리자/기본 CRUD 성격 API로 유지한다.

### 3.3 지갑 기준 사용
* **엔드포인트:** `POST /api/v1/point-wallets/{walletId}/use`

### 3.4 인증 회원 기준 충전
* **엔드포인트:** `POST /api/v1/points/charge`
* **헤더:** `Authorization: Bearer {accessToken}`, `Idempotency-Key: {uuid}`
* **Request Body 예시:**
  ```json
  {
    "amount": 50000,
    "paymentMethod": "CARD"
  }
  ```
* **Response Body 예시:**
  ```json
  {
    "balance": 50000,
    "lastChargedAt": "2026-08-04T01:30:00"
  }
  ```

> [!IMPORTANT]
> 동일 회원이 같은 `Idempotency-Key` 로 재호출하면 새 이력을 만들지 않고 기존 충전 결과를 그대로 반환합니다.

---

## 4. 구현 메모

* 지갑 잔액 변경은 `PaymentRepository.findByIdForUpdate(...)` 또는 `findByMemberIdForUpdate(...)` 로 비관적 락을 사용한다.
* 인증 회원 기준 충전은 `PointHistory` 에 `CHARGE` 이력을 남긴다.
* `point_history(member_id, idempotency_key)` 유니크 제약으로 멱등성을 보강한다.
* 자동 타임아웃 환불은 지갑 비관적 락 후 `REFUND + referenceId`를 확인하고 `delivery-timeout-refund:{deliveryRequestId}` 키로 이력을 저장한다.
* `point_history(type, reference_id)` 유니크 인덱스가 동일 배송의 중복 환불을 DB에서도 차단한다.
* 고객 직접 취소는 고객 `REFUND`와 배정 배송원 `COURIER_COMPENSATION`을 하나의 트랜잭션에서 처리한다. 두 이력은 동일 `referenceId`를 사용하되 유형이 달라 각각 한 번만 기록된다.
* 상태별 금액 계산은 [단계별 취소 수수료·배송원 보상 설계서](./단계별-취소-수수료-배송원-보상-기능-설계서.md)를 따르고, 전역 락 순서는 상위 결정 문서인 [ADR-0004](../adr/0004-고객-취소-원자적-포인트-정산.md)를 SSOT로 따른다.

---

## 5. 결제 확인 화면 즉시 충전 플로우 (Issue #247)

* **화면:** `payment-confirmation.html`
* **트리거:** 보유 포인트(`walletBalance`)가 총 결제 금액(`totalFee`)보다 적을 때, 안내 배너에 부족액을 표시하고 `결제 PIN 입력하기` 버튼 대신 `부족한 포인트 충전하기` 버튼을 노출한다.
* **충전 모달:** 별도 페이지 이동 없이 화면 내 모달로 충전 금액(부족액 자동 프리필)과 결제 수단을 입력받는다.
* **API 연동:** 모달 제출 시 **3.4 인증 회원 기준 충전 API**(`POST /api/v1/points/charge`)를 재사용한다. 이슈 초안에서 언급된 `POST /api/v1/point-wallets/{walletId}/charge`(3.2)는 소유권 검증과 `PointHistory` 기록이 없는 관리자용 API라 사용자 결제 플로우에는 부적합하여 채택하지 않았다.
* **충전 후 처리:** 응답의 `balance` 값으로 화면 상태를 즉시 재계산해 배너를 `충분` 상태로 전환하고 `결제 PIN 입력하기` 버튼을 다시 노출한다.

---

## 6. 검증 명령어

```bash
./gradlew.bat test --tests "*PaymentServiceTest" --tests "*PointControllerTest"
./scripts/verify.sh
```
