...
* **설명:** 실제 배송 요청을 생성하지 않고, 출발지·도착지 좌표·물품 무게·물품 크기로 예상 요금을 미리 계산한다.
  ...
  3.1의 `feePoint` 계산식(거리×100원+무게×10원, 클라이언트가 준 raw distance 사용)과는 별개의 공식이며 통합하지 않는다.
* **요청 바디 (Request Body):**
  ```json
  {
...
- [ ] `ItemSize`(SMALL/MEDIUM/LARGE) 크기 할증 필드 추가 및 `sizeSurcharge` 계산 로직 반영
- [ ] 크기 할증 관련 테스트(무게+크기 복합 계산, 크기 누락 400) 추가
