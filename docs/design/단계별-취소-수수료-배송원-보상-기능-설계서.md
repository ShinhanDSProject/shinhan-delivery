...
sequenceDiagram
    ...
    D->>D: 소유권·상태·정산액 검증
    D->>DB: Matching 비관적 락
    D->>DB: Vehicle 비관적 락 및 가용화
    D->>P: 고객 환불 + 배송원 보상
    P->>DB: 지갑 잠금 및 멱등 이력 저장
    D->>DB: Matching/Delivery 상태 갱신
    D-->>C: 실제 수수료·환불·보상 결과
```

## 동시성·멱등성

- 락 순서는 [ADR-0004](../adr/0004-고객-취소-원자적-포인트-정산.md)에 따라 `DeliveryRequest → Matching → Vehicle → 고객 PointWallet → 배송원 PointWallet`로 고정한다.
