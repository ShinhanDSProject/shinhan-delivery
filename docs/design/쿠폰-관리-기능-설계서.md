...
* **ADM-NFR-02 (작업 이력 감사Log):** 관리자가 쿠폰을 생성하거나  `상태를 변경(INACTIVE) `할 때 작업자 ID, 일시, 변경 전/후 상태가 감사 로그(Audit Log)로 기록되어야 한다.
...
%% ======================================================
  COUPON {
    ...
    BigDecimal discount_value "할인 값 (금액 또는 비율)"
    BigDecimal min_delivery_fee "최소 배송비 조건"
    BigDecimal max_discount_amount "최대 할인 금액 한도"
