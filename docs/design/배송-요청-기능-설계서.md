...
metadata:
  version: "1.1.0"
  ssot_owner: "docs/design/배송-요청-기능-설계서.md"
...
erDiagram
    ...
    DELIVERY_REQUEST {
        ...
        String delivery_instruction_type "현장 전달 방식"
        String entrance_code "권한 제한 민감정보"
        String unit_detail "동/호수"
        String delivery_note "기타 요청"
        String delivery_reference_photo_url "입구 참고 사진"
    }
```
...
- [x] 전달 방식과 조건부 현장 정보 저장 및 결제 요청 연동 (#279)
- [x] 공동현관 출입번호를 권한 검사된 상세 API로만 조회하도록 제한 (#279)
- [x] 공통 디자인 시스템·Thymeleaf 프래그먼트 기반 모바일 입력 UI 적용 (#279)
...
```mermaid
sequenceDiagram
    participant Customer as 고객 UI
    participant Controller as DeliveryController
    participant Service as DeliveryService
    participant Repository as DeliveryRequestRepository
    Customer->>Controller: 전달 지침을 포함한 배송 결제 요청
    Controller->>Service: 인증 회원 ID + DTO
    Service->>Repository: 배송 요청 저장
    Note over Repository: 출입번호는 목록/오퍼 응답에서 제외
    Service-->>Customer: 권한 검사 후 상세 조회에서만 반환
```

> [!IMPORTANT]
> 공동현관 출입번호는 비밀번호와 동일한 인증 수단은 아니지만 현장 접근을 가능하게 하는 민감정보다. 로그·예외 메시지·배송 대기열·실시간 오퍼에는 포함하지 않고, 기존 고객 본인 또는 배정 배송원 인가가 적용된 상세 API에서만 제공한다.

`deliveryInstructionType`을 생략한 기존 클라이언트는 `NONE`으로 저장되므로 기존 API 계약을 유지한다. `ENTRANCE_CODE`는 `entranceCode`, `CUSTOM`은 `deliveryNote`가 반드시 필요하며, 잘못된 조합은 공통 `INVALID_INPUT_VALUE` 응답으로 처리한다.

전달 지침은 배송 요청 생성 시 확정되며 기존 주소 수정 API의 변경 대상에 포함하지 않는다. 브라우저 화면 간 초안은 탭 수명에 한정된 `sessionStorage`를 사용하고, 결제가 성공하면 출입번호 값을 즉시 제거한다.

WHY: 유형을 자유 문자열로 저장하면 오탈자와 조건부 검증 분기가 늘어나므로 Enum을 선택했다. 출입번호 암호화 저장도 검토했지만 현재 프로젝트에는 운영 키 관리 체계가 없어 애플리케이션 코드에 키를 추가하는 편이 더 위험하다. 우선 응답 최소화와 접근 제어를 적용하고, 키 관리 체계 도입 시 컬럼 암호화를 후속 적용한다.
...
기대 결과: Flyway 파일명·DDL, Spotless, ArchUnit, 테스트와 JaCoCo 게이트가 모두 성공한다.
