...
- [5. 신한 개척자 배송 도메인 상태 머신 그래프 (Domain State Graph)](#5-신한-개척자-배송-도메인-상태-머신-그래프-domain-state-graph)
...
`shinhan-gaecheokja` 프로젝트의 품질 검증 하네스(`./scripts/verify.sh`)는 그래프 엔지니어링의 **조건부 회귀(Cyclic Edge) 및 자가 치유(Self-Healing)** 원리를 구현한 대표적 사례입니다.
...
## 5. 신한 개척자 배송 도메인 상태 머신 그래프 (Domain State Graph)
