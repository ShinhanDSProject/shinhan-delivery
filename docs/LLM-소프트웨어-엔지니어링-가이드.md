...
7. [5️⃣ 에이전트 방법론, 그래프 엔지니어링 & 디자인 패턴 (Agentic Workflows, Graph Engineering & EDD)](#7-5️⃣-에이전트-방법론-그래프-엔지니어링--디자인-패턴-agentic-workflows-graph-engineering--edd)
...
│  6. 그래프 엔지니어링  ──▶ 노드/엣지 기반 "복잡한 에이전트 워크플로우 오케스트레이션" │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| **🕸️ 그래프 엔지니어링** | AI 에이전트의 역할(Node)과 실행 경로(Edge)를 **네트워크로 구조화하여 복잡한 비즈니스 로직을 제어하기 위해** | 에이전트가 단일 루프에서 길을 잃거나 무한 루프에 빠져 작업이 꼬임 |
...
이 변화를 주도하는 6대 엔진이 바로 **프롬프트 엔지니어링, 컨텍스트 엔지니어링, 하네스 엔지니어링, 피드백 루프, 에이전트 워크플로우, 그래프 엔지니어링**입니다.
...
## 7. 5️⃣ 에이전트 방법론, 그래프 엔지니어링 & 디자인 패턴 (Agentic Workflows, Graph Engineering & EDD)
...
### 🕸️ 2) 그래프 엔지니어링 (Graph Engineering & Agentic Orchestration)

#### 💡 개념
**그래프 엔지니어링(Graph Engineering)**은 단순한 단일 에이전트 루프의 한계를 극복하고, AI 에이전트의 수행 단계(**노드 Node: 역할/도구/검증기**)와 작업 흐름/조건부 분기(**엣지 Edge: 성공/실패/회귀/병렬 분기**)를 네트워크(Graph/DAG) 형태로 설계하여 복잡한 비즈니스 프로세스를 완벽히 구조화하고 제어하는 최신 아키텍처 패러다임입니다.

```mermaid
graph TD
    UserReq["사용자 요청 (User Request)"] --> Node1["1. 리서치 & 설계 노드 (Researcher)"]
    Node1 --> Node2["2. 코드 작성 노드 (Coder)"]
    Node2 --> Node3{"3. 하네스 검증 노드 (verify.sh)"}
    Node3 -- "실패 (Fail)" --> Node4["4. 자가 치유 노드 (Self-Healing Fixer)"]
    Node4 --> Node2
    Node3 -- "성공 (Pass)" --> Node5["5. 셀프 코드 리뷰 & 자산화 노드"]
    Node5 --> Node6["6. PR 생성 & CI 머지 노드"]
```

#### 🧩 3대 핵심 구성 요소
1. **노드 (Node - 작업 단위):** 독립적인 미션을 수행하는 에이전트/도구/검증기 (예: `기획 에이전트`, `코더`, `하네스 검증기`, `사용자 승인`)
2. **엣지 (Edge - 이동 경로 & 조건):** 노드 간의 상태 기반 이동 경로 (예: `성공 ➔ 다음 노드`, `실패 ➔ 수정 노드로 회귀`)
3. **상태 (Shared State - 공유 메모리):** 노드 사이를 이동하며 업데이트되는 공유 컨텍스트 메모리 (예: 수정 파일 목록, 스택 트레이스)

#### 🎯 그래프 엔지니어링을 쓰는 이유 (WHY)
- **무한 루프 낭비 방지:** 조건부 엣지를 통한 명확한 회귀/종료 조건 정의로 리소스 낭비 차단
- **사람과 에이전트의 하이브리드 협업 (Human-in-the-loop):** 특정 노드에 사람의 의사결정/승인 단계를 자연스럽게 통합 가능
- **대표 도구:** `LangGraph`, `GraphRAG`, `Google Antigravity Multi-Agent Orchestration`

---

### 🧪 3) 평가 기반 개발 (Eval-Driven Development, EDD)
