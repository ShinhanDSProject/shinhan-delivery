...
> **문서 버전:** v7.0 (Planning-First & End-to-End Automated Workflow Deck)  
> **최종 수정일:** 2026-08-21  
> **대상:** 신한 딜리버리(shinhan-delivery) 프로젝트 발표자 및 팀원 전체  
> **목적:** MVP 기획서 및 화면 설계서 사전 작성을 시작으로 GitHub Issue 연동 AI 페어 프로그래밍, Thymeleaf SSR/CSR 렌더링 오용 극복, HTML/CSS/JS 파일 분리 모듈화, 자가 치유 피드백 루프 구축 경험을 담은 발표 시나리오 슬라이드 덱입니다.
...
[Slide 2] 🔍 Phase 1: 화려한 성공 뒤의 5대 현실 문제점 (HTML/CSS/JS 파일 비분리 & SSR 외면 CSR AJAX 남용)
   ↓ "하지만 AI 맹목적 의존과 인라인 코드 오염, FOUC 0.5초 현상이 발생하는 치명적 반전이 찾아왔습니다."
...
* **주요 발표 내용:**
  1. **사전 기획-이슈 기반 AI 페어 프로그래밍 4단계 체계**:
     ...
     - **[2단계: 이슈 발행 및 기획]** 기획/화면 설계서 기반 **GitHub Issue 발행** ➔ `/plan <이슈번호>` 지침 기반 **AI 자동 이슈 분석 및 설계 작성** ➔ **개발자 승인 Checkpoint**
...
> 🎤 **발표 멘트 포인트:**  
> *"사전에 MVP 기획서와 화면 설계서를 훌륭히 작성하고 AI 페어 프로그래밍을 시작했음에도, 사람이 직접 의사결정을 내리지 않으면 AI가 단일 파일 안에서 코드를 인라인으로 섞어버리거나 SSR 템플릿의 장점을 무시하는 설계 결함을 유발하게 됩니다."*
...
1. **[수칙 1] 선(先) 기획 및 사람이 직접 기술적 의사결정 주도 (Planning-First & Human Decision)**
   - 개발 전 **MVP 기획서와 화면 설계서**를 선제 정의하고, 레이어 분리, 무상태성, 유효성 검사 등 핵심 방향성을 사람이 직접 설계하고 지시할 것.
...
* **💡 Final Key Lesson:**  
  `"AI 도구는 개발자를 대체하는 것이 아니라, 사전 기획과 화면 설계, 아키텍처를 고민하고 기술적 의사결정을 내리는 진짜 수석 엔지니어의 가치를 더욱 돋보이게 만든다."`
