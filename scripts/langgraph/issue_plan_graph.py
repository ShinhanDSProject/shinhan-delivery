...
def issue_analyzer_node(state: IssuePlanState) -> IssuePlanState:
    print(f"📋 [LangGraph 노드 1: 이슈 분석기] {state['issue_title']} 분석 중...")
    state["status"] = "ANALYZED"
    state["affected_components"] = ["Controller", "Service", "Repository", "Entity", "TestHarness"]
    state["history"].append(f"1단계: 이슈분석 완료 ({state['issue_title']})")
    return state
...
def plan_generator_node(state: IssuePlanState) -> IssuePlanState:
    ...
    plan_md = f"""# Implementation Plan - {state['issue_title']}
...
def run_issue_plan_graph(issue_title: str):
    print("======================================================")
    ...
    state: IssuePlanState = {
        ...
        "affected_components": [],
        "plan_content": "",
        "is_plan_approved": True, # Simulated user approval
        "fix_attempts": 0,
        "max_fix_attempts": 3,
        "status": "INIT",
        "history": []
...
if __name__ == "__main__":
    issue = sys.argv[1] if len(sys.argv) > 1 else "Issue #108: POST /api/v1/deliveries/pay 결제 API 구현"
    run_issue_plan_graph(issue)
